...
function applyI18n() {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        if (id)
            el.textContent = getString(id);
    });
    document.querySelectorAll('[data-i18n-title]').forEach(el => {
        const id = el.getAttribute('data-i18n-title');
        if (id)
            el.setAttribute('title', getString(id));
    });
    const docTitle = document.getElementById('doc-title');
    if (docTitle)
        docTitle.textContent = getString('title_page');
    const appTitle = document.getElementById('app-title');
    if (appTitle)
        appTitle.textContent = getString('app_title');
    const optHome = document.getElementById('opt-home');
    if (optHome)
        optHome.textContent = getString('menu_home');
    const optProfile = document.getElementById('opt-profile');
    if (optProfile)
        optProfile.textContent = getString('menu_profile');
    const optLogout = document.getElementById('opt-logout');
    if (optLogout)
        optLogout.textContent = getString('menu_logout');
    const optionsTrigger = document.getElementById('options-trigger');
    if (optionsTrigger)
        optionsTrigger.setAttribute('aria-label', getString('aria_options'));
    const langMenuEl = document.getElementById('lang-menu');
    if (langMenuEl)
        langMenuEl.setAttribute('aria-label', getString('aria_language'));
    ...
    document.querySelectorAll('button[data-i18n]').forEach(btn => {
        ...
        if (id)
            btn.textContent = getString(id);
    });
    const stepPrev = document.querySelector('.step-prev');
    if (stepPrev)
        stepPrev.setAttribute('aria-label', getString('btn_back'));
    const stepNext = document.querySelector('.step-next');
    if (stepNext)
        stepNext.setAttribute('aria-label', getString('btn_next'));
    const floatBack = document.getElementById('floating-back');
    if (floatBack)
        floatBack.setAttribute('aria-label', getString('btn_back'));
}
...
function showStep(index) {
    ...
    if (frame) {
        if (activeStep && activeStep.hasAttribute('data-pillar-id'))
            frame.classList.add('show-step-arrows');
        else
            frame.classList.remove('show-step-arrows');
    }
...
function goNext() {
    ...
    if (!inputCheck.ok) {
        ...
        const err = inputCheck;
        showMessage(err.message, true);
        if (err.missingNames)
            markStepValidationErrors(step, err.missingNames);
        return;
    }
...
function renderSummary(data, _fromPillar) {
    ...
    if (rec && recordHasAnyData(rec)) {
        ...
        if (rec.q1 && rec.q1.trim())
            html += '<div class="summary-block"><p class="summary-title">' + escapeHtml(getString('label_q1')) + '</p><p class="summary-value">' + escapeHtml(rec.q1.trim()) + '</p></div>';
        if (rec.q2 && rec.q2.trim())
            html += '<div class="summary-block"><p class="summary-title">' + escapeHtml(getString('label_q2')) + '</p><p class="summary-value">' + escapeHtml(rec.q2.trim()) + '</p></div>';
        if (rec.q3 && rec.q3.trim())
            html += '<div class="summary-block"><p class="summary-title">' + escapeHtml(getString('label_q3')) + '</p><p class="summary-value">' + escapeHtml(rec.q3.trim()) + '</p></div>';
        if (adviceText)
            html += '<div class="summary-block"><p class="summary-title">' + escapeHtml(getString('summary_advice')) + '</p><p class="summary-value">' + escapeHtml(adviceText) + '</p></div>';
        html += '</div>';
    }
...
function openHistory() {
    ...
    if (backBtn)
        backBtn.textContent = getString('btn_back');
    if (deleteBtn)
...
function renderHistoryList() {
    ...
    panel.querySelectorAll('.record-link').forEach(el => {
        el.addEventListener('click', function (e) {
            if (e.target.classList.contains('export-checkbox') || e.target.closest('.export-check-wrap'))
                return;
            ...
            const exportBtn = document.getElementById('btn-history-export');
            if (backBtn)
                backBtn.textContent = getString('btn_back_to_list');
            if (deleteBtn)
...
function buildRecordHtml(item) {
    ...
    PILLAR_IDS.forEach(pillarId => {
        ...
        if (rec.poderoso && String(rec.poderoso).trim())
            html += '<p><strong>' + escapeHtml(getString('summary_most_powerful_behavior')) + '</strong></p><p>' + escapeHtml(rec.poderoso.trim()) + '</p>';
        if (rec.q1 && String(rec.q1).trim())
            html += '<p><strong>' + escapeHtml(getString('label_q1')) + '</strong></p><p>' + escapeHtml(rec.q1.trim()) + '</p>';
        if (rec.q2 && String(rec.q2).trim())
            html += '<p><strong>' + escapeHtml(getString('label_q2')) + '</strong></p><p>' + escapeHtml(rec.q2.trim()) + '</p>';
        if (rec.q3 && String(rec.q3).trim())
            html += '<p><strong>' + escapeHtml(getString('label_q3')) + '</strong></p><p>' + escapeHtml(rec.q3.trim()) + '</p>';
...
function bindNav() {
    ...
    if (btnViewSummary)
        btnViewSummary.addEventListener('click', goToSummary);
    ...
    if (btnViewRecordsSummary)
        btnViewRecordsSummary.addEventListener('click', openHistory);
    const btnViewRecordsStart = document.getElementById('btn-view-records-start');
    if (btnViewRecordsStart)
        btnViewRecordsStart.addEventListener('click', openHistory);
    ...
    if (btnHistoryExport)
        btnHistoryExport.addEventListener('click', exportHistoryHTML);
    const btnHistoryDelete = document.getElementById('btn-history-delete');
    if (btnHistoryDelete)
        btnHistoryDelete.addEventListener('click', deleteAllRecords);
    ...
    if (messageBox)
        messageBox.addEventListener('click', function (e) { e.stopPropagation(); });
    const confirmBox = document.getElementById('confirm-box');
    if (confirmBox)
        confirmBox.addEventListener('click', function (e) { e.stopPropagation(); });
    ...
    if (optionsTrigger && optionsDropdown) {
        ...
        if (optHomeEl)
            optHomeEl.addEventListener('click', function () { closeOptionsMenu(); });
        const optProfileEl = document.getElementById('opt-profile');
        if (optProfileEl)
            optProfileEl.addEventListener('click', function () { closeOptionsMenu(); });
        const optLogoutEl = document.getElementById('opt-logout');
        if (optLogoutEl)
            optLogoutEl.addEventListener('click', function () { closeOptionsMenu(); });
    }
    const langMenuEl = document.getElementById('lang-menu');
    if (langMenuEl) {
    ...
    if (typeof window !== 'undefined' && window.Capacitor?.Plugins?.App) {
        try {
            ...
            Cap.addListener('backButton', function () {
                ...
                if (landingEl && landingEl.classList.contains('active')) {
                    if (Cap.exitApp)
                        Cap.exitApp();
                    return;
                }
                if (currentStepIndex > 0) {
                    ...
                    window.dispatchEvent(new CustomEvent('app:go-landing'));
                }
            });
        }
        catch (e) { }
...
/**
 * Handle “back” inside the FASTER wizard (steps + historial). Returns true if navigation was
 * handled so the shell should not also show the landing page.
 */
export function performFasterBack() {
    const faster = document.getElementById('view-faster');
    if (!faster?.classList.contains('active'))
        return false;
    const activeStep = document.querySelector('#wizard .step.active');
    const sid = activeStep?.id;
    if (sid === 'step-history') {
        const btn = document.getElementById('btn-history-back');
        if (btn) {
            btn.click();
            return true;
        }
...
/** Reset FASTER wizard to the intro step (after opening from the home landing). */
export function goToFasterIntro() {
    hideMessage();
    showStep(0);
}
export function bootstrapFasterApp() {
    loadLang(currentLang).then(function () {
        applyI18n();
        bindNav();
