...
import type { AppContext } from '../../shell/app-context.js';
import type { SwordDevotionalDTO, SwordSessionRecord, SwordStepId } from './sword.types.js';

const STEP_ORDER: SwordStepId[] = ['scripture', 'write', 'observation', 'reflection', 'do'];

const STEP_LABEL: Record<SwordStepId, string> = {
...
function getPromptForStep(dev: SwordDevotionalDTO, key: SwordStepId): string {
  const s = dev.steps;
  switch (key) {
    case 'write':
...
function emptySteps(): Record<SwordStepId, string> {
  return {
    scripture: '',
    write: '',
    observation: '',
...
export function bootstrapSwordFeature(ctx: AppContext): void {
  ...
  let catalog: SwordDevotionalDTO[] = [];
  let current: SwordDevotionalDTO | null = null;
  let session: SwordSessionRecord | null = null;
  let stepIndex = 0;
  ...
  function renderStep() {
    if (!stepContent || !current || !session) return;
    ...
    const field = document.getElementById('sword-field') as HTMLTextAreaElement | null;
    if (field) {
      field.addEventListener('input', () => {
        if (!session) return;
        if (key === 'scripture') session.steps.scripture = field.value;
        else session.steps[key] = field.value;
      });
    }

    if (btnBack) btnBack.style.display = stepIndex === 0 ? 'none' : '';
    if (btnNext) btnNext.style.display = stepIndex >= STEP_ORDER.length - 1 ? 'none' : '';
    if (btnSave) btnSave.style.display = stepIndex >= STEP_ORDER.length - 1 ? '' : 'none';
  }

  function openWizard(dev: SwordDevotionalDTO) {
    current = dev;
      ...
      if (listEl) {
          ...
          .map(d => {
            const tags =
              d.tags && d.tags.length
                ...
                : '';
            return (
              '<button type="button" class="sword-pick" data-id="' +
              escapeHtml(d.id) +
        ...
        listEl.querySelectorAll('.sword-pick').forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            if (dev) openWizard(dev);
          });
        });
      }
    } catch (e) {
      const msg =
        e instanceof HttpJsonError
          ? e.message
          : e instanceof Error
            ? e.message
  ...
  async function renderSessionList() {
    ...
    sessionsEl.innerHTML = rows
      ...
      .map(
        r =>
          '<div class="sword-session-row" data-id="' +
          escapeHtml(r.id) +
          '"><span>' +
