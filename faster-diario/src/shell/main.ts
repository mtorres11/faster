...
type FeatureId = 'landing' | 'faster' | 'sword' | 'podcasts' | 'bible' | 'profile';
...
function showFeature(id: FeatureId): void {
...
function updateFloatingBackVisibility(): void {
...
function handleFloatingBack(): void {
  if (performFasterBack()) return;
  if (document.getElementById('view-landing')?.classList.contains('active')) return;
  showLanding();
}

async function bootstrapProfile(ctx: ReturnType<typeof createAppContext>): Promise<void> {
...
function wireLandingAndMenu(ctx: ReturnType<typeof createAppContext>): void {
...
function wireLogin(ctx: ReturnType<typeof createAppContext>): void {
...
function startAuthenticatedApp(ctx: ReturnType<typeof createAppContext>): void {
